...
// npx hardhat run scripts/strategies/deployAnyPool.js --network sepolia
async function main() {
    ...
    const baseToken = process.env.LINK_SEPOLIA_ADDRESS; // baseToken trên Sepolia
    const quoteToken = process.env.WETH_SEPOLIA_ADDRESS; // quoteToken trên Sepolia
    ...
    const nameParam = "MoneyFi UniswapV2 LINK/WETH";
    const symbolParam = "MFUWV2";
    const minimumSwapAmount = ethers.parseUnits("0.01", 6); // 0.01 USDC
    const slippageWhenSwapAsset = 100; // 0.5% slippage

    if (!baseToken || !quoteToken || !uniswapRouter || !uniswapFactory || !router || !crossChainRouter) {
